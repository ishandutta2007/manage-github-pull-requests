...
def fetch_pull_requests(username, include_forks=False):
    ...
    if auth_user and auth_user.lower() == username.lower():
        print(
            f"Fetching all repositories (public & private) for authenticated user: {username}..."
        )
    ...
    repos, repos_from_cache = get_api_data(
        repos_url, params=params, cache_category="repos", cache_id=username
    )
    ...
    for idx, repo in enumerate(filtered_repos, 1):
        ...
        print(
            f"   📊 Stats: Total: {len(prs)} | 🟢 Open: {len(open_prs)} | 🔴 Closed: {len(closed_prs)} | ⚠️ Conflicts: {len(repo_conflicts)}"
        )
        ...
        if open_prs:
            ...
            for pr in open_prs:
                ...
                print(
                    f"      {pr['number']:<5} | {title_truncated:<50} | {pr['user']['login']:<20} | {conflict_status}"
                )
            print("      " + "-" * 110 + "\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Fetch all pull requests for all repos of a GitHub username."
    )
    parser.add_argument("username", nargs="?", help="GitHub username to fetch PRs for")
    parser.add_argument(
        "--username",
        dest="username_flag",
        help="GitHub username to fetch PRs for (alternative to positional)",
    )
    parser.add_argument(
        "--include-forks",
        action="store_true",
    ...
    try:
        all_prs, conflicted_prs = fetch_pull_requests(
            username, include_forks=args.include_forks
        )
        ...
        if conflicted_prs:
            ...
            # INTERACTIVE SECTION
            print(
                "🛠️  Interactive Cleanup: Would you like to close these conflicted PRs?"
            )
